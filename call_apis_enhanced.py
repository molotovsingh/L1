"""Enhanced API calling functions for OpenAI models with flexible parsing"""
...
# Import the flexible parser from our new utility module or use the local version
try:
    from src.utils.parsers import flexible_response_parser
except ImportError:
    # Local implementation of the flexible parser
...
def call_tier_a_api(prompt: str, api_key: Optional[str], model_name: str) -> Tuple[Union[List[str], str, None], Optional[str], Optional[datetime]]:
    ...
    Enhanced with flexible parsing to extract structured labels from various formats.
        ...
        try:
            ...
            if len(response.choices) > 0:
                ...
                if content:
                    ...
                    # Use the flexible parser to extract structured labels if possible
                    extracted_labels = flexible_response_parser(content)
                    if extracted_labels:
                        st.success(f"✅ Successfully extracted {len(extracted_labels)} labels using flexible parser")
                        return extracted_labels, raw_content, api_timestamp
                    
                    # If flexible parser didn't find anything, fall back to returning raw text
            ...
            if is_o_series:
                ...
                try:
                    ...
                    if len(fallback_response.choices) > 0 and fallback_response.choices[0].message.content:
                        ...
                        # Try parsing with the flexible parser
                        extracted_labels = flexible_response_parser(fallback_content)
                        if extracted_labels:
                            st.success(f"✅ Successfully extracted {len(extracted_labels)} labels from fallback response")
                            return extracted_labels, fallback_content, api_timestamp
                            
                        return fallback_content.strip(), fallback_content, api_timestamp
...
def call_openai_api(prompt: str, api_key: Optional[str], model_name: str) -> Tuple[Optional[str], Optional[str], Optional[datetime]]:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if is_o_series:
                st.warning("""
                ⚠️ **O-series Model Failed (Tier-B)**
