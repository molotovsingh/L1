...
from src.utils.parsers import flexible_response_parser, extract_structured_data
...
def call_perplexity_api_tier_a(
    prompt: str, 
    api_key: Optional[str], 
    model_name: str = "sonar"
) -> Tuple[Union[List[str], str, None], Optional[str], Optional[datetime]]:
    """
    ...
    Returns:
        Tuple containing:
        - Union[List[str], str, None]: Processed API response content (can be list of labels, raw string, or None on failure)
...
def call_perplexity_api_tier_b(
    prompt: str, 
    api_key: Optional[str], 
